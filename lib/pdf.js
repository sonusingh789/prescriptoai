...
import fs from 'fs'; // Add this import for reading the image file
...
export async function buildPrescriptionPDF(prescriptionData) {
  ...
  let logoImage = null;
  try {
    const logoPath = 'C:/Users/Sonu-PC/Desktop/prescriptoai/public/logo.png'; 
    const logoBytes = fs.readFileSync(logoPath);
    logoImage = await doc.embedPng(logoBytes);
  } catch (error) {
    console.warn('Logo image not found, proceeding without logo:', error.message);
  }
  ...
  function drawHeader(isContinuedPage = false) {
    ...
    // Draw logo if available (instead of hospital name text)
    if (logoImage) {
      // Scale logo to be very small and compact
      const logoScale = 0.07; // Small logo to save space
      const logoDims = logoImage.scale(logoScale);
      
      // Position logo at the top
      page.drawImage(logoImage, {
        x: leftX,
        y: y - logoDims.height, // Position logo at top
        width: logoDims.width,
        height: logoDims.height,
      });
      
      // Show date at the same level as logo
      const meta = `${date}`;
      ...
      page.drawText(metaFit.text, { 
        x: rightX - metaW, 
        y: y - 2, // Position near the top, aligned with logo
        size: metaFit.size, 
        font: helvetica, 
        color: COLORS.muted 
      });
      
      // Adjust y position minimally - just enough to clear the logo
      y = y - logoDims.height - 2; // Minimal spacing
    } else {
      // If no logo, just show date
      const meta = `${date}`;
      const metaFit = fitTextToWidth(meta, 240, helvetica, SIZES.headerMeta);
      const metaW = helvetica.widthOfTextAtSize(metaFit.text, metaFit.size);
    ...
    // Draw the colored line with minimal gap
    y -= 8; // Minimal gap before line
    page.drawRectangle({ 
      x: MARGIN_X, 
      y: y, 
      width: A4_WIDTH - MARGIN_X * 2, 
    ...
    // Minimal spacing after the line
    y -= 8;
  }
